...
def schema(args):
    ...
    if args['--db_per_file'] or len(db_list) == 1:
        ...
        subprocess.call('mysqldump {} --no-data --force --quote-names --dump-date --opt --single-transaction --events --routines --triggers --databases {} --result-file={}/{}.sql'.format(' '.join(connect_list),' '.join(db_list),args['--output_dir'],'alldb'))
